...
module.exports = {
  ...
  let embed =  new MessageEmbed()
      .setTitle('HELP MENU')
      .setColor('RANDOM')
      .setFooter(`${process.env.prefix}help <category> <command>`, message.author.displayAvatarURL())
    ...
    if (!args[0]) {
      ...
      if (args[0]) {
        if (args[1]) {
          if (Object.keys(commands).includes(args[1].toLowerCase()) || Object.keys(commands).map(c => commands[c].aliases || []).flat().includes(args[1].toLowerCase())) {
            ...
            if (commands[command].category === args[0]) {
              ...
              embed
                ...
                .addField('FORMAT', `\`\`\`${process.env.prefix}${commands[command].format}\`\`\``);
            } else {
              embed
                .setColor('RED')
                .setDescription('This command does not exist in the category you specified.');
            }
          } else {
            embed
              .setColor('RED')
          ...
          const cmdInCat = cmds.cmds.filter(name => name.category.includes(args[0]))
          const cmdToAdd = []
          cmdInCat.forEach(c => {
        ...
        embed
          .setColor('RED')
          .setDescription('This category does not exist. Please use the help command without specifying any categories to list them all.');
      }
    }
